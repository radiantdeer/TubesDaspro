...
interface
	...
	procedure lihattransaksi (var noakun:string; Trek : lrekonline; Ttrans : ltrans; Ttrf : ltrf; Tbayar : lpembayaran; Tbeli : lpembelian);
...
implementation
	...
	begin
		...
		readln(noakun);
			for i1:=1 to (Ttrans.Neff) do
			begin
				if(noakun=(Ttrans.list[i1].noakun)) then
				begin
					tanggal2:=Ttrans.list[i1].tgl;
					for i2:=1 to (Ttrf.Neff) do
					begin
						if(noakun=(Ttrf.list[i2].asal)) then
						begin
							tanggal3:=Ttrf.list[i2].tgl;
							for i3:=1 to (Tbayar.Neff) do
							begin
								if(noakun=(Tbayar.list[i3].noakun)) then
								begin
									tanggal4:=Tbayar.list[i3].tgl;
									for i4:=1 to (Tbeli.Neff) do
									begin
										if(noakun=(Tbeli.list[i4].noakun)) then
										begin {cek satu-satu no akun itu ada ato tidak di tiap array}
											tanggal5:=Tbeli.list[i4].tgl;
											s1:=gettgl(tanggal1);
											s2:=getbln(tanggal1);
											s3:=getthn(tanggal1);
											s4:=gettgl(tanggal2);
